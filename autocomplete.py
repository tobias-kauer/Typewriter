...
def build_prompt(prompt):
    ...
    return (
        "Context: The visitor is typing a review on a typewriter at the exhibition \"mind the gap\" "
        "by the class \"Design and Narrative Media\" at the Angewandte Festival in Vienna. "
        "NOTE: The exhibition consists of only ONE single room containing multiple individual projects.\n\n"
        "Instruction: Respond immediately with the completion. Do NOT use <think> tags, do NOT think step-by-step, and do NOT output any reasoning.\n"
        "Complete the input seamlessly until the first period ('.') while strictly enforcing the SENTIMENT FLIP:\n\n"
        "CRITICAL SENTIMENT RULES (MANDATORY):\n"
        "1. JUDGE ONLY THE END: Always evaluate the sentiment of the visitor's MOST RECENT words (how they end their text), not how they started.\n"
        "2. IF ENDING IS POSITIVE (e.g. user changed mind to positive): You MUST immediately force a turn to a NEGATIVE, critical continuation (using 'aber', 'trotzdem', 'obwohl', 'but').\n"
        "3. IF ENDING IS NEGATIVE (e.g. user changed mind to negative): You MUST immediately force a turn to a POSITIVE, impressed continuation (using 'doch', 'trotzdem', 'aber', 'but').\n"
        "4. IF INPUT IS NEUTRAL/FACTUAL: Write ecstatic, over-the-top, exuberant praise of the exhibition.\n\n"
        "FORMATTING, STYLE & LANGUAGE:\n"
        "- LANGUAGE LOCK: Always write 100% in the exact same language as the input. If the input is English, write English. If German, write German. Never switch languages.\n"
        "- Style: Mimic the visitor's register exactly. If they write in casual youth slang (e.g., 'Bock', 'geil', 'Absturz', 'ur cool'), match it. Do NOT write like an academic critic; avoid dramatic words (like 'Meisterleistung', 'Spektakel', 'Brillanz'). Keep it grounded.\n"
        "- If input ends mid-sentence with a finished word (with space): Continue in lowercase starting with a comma (e.g., ', weil...').\n"
        "- If input ends mid-word (no space): Complete that word first (e.g., 'gep' -> 'ackt').\n"
        "- German Inversion (V2 Word Order): If ending in 'Trotzdem ', 'Dennoch ', 'Deshalb ', 'Doch leider ', start immediately with the finite verb.\n"
        "- CRITICAL: Do NOT copy, repeat, or echo any part of the input. Output ONLY the continuation.\n\n"
        "Input: I absolutely love the \"mind the ga\n"
        "Output: p\" exhibition, but the layout makes it incredibly hard to navigate.\n\n"
        "Input: Ich finde die Ausstellung eigentlich voll geil, aber \n"
        "Output: manche Installationen sind leider komplett verbuggt.\n\n"
        "Input: Ich dachte erst, die Schau ist voll langweilig, aber das Sound-Projekt hat mich voll abgeholt, obwohl \n"
        "Output: die Kopfhörer dort total dreckig waren.\n\n"
        "Input: Ich habs ur cool gefunden hier zu sein. Alle Projekte waren ur nice und \n"
        "Output: trotzdem war die Luft so unerträglich stickig, dass ich schnell wieder weg musste.\n\n"
        "Input: J'aime beaucoup l'exposition, mais \n"
        "Output: l'organisation générale est vraiment chaotique.\n\n"
        "Input: La presentación ha sido una decepción total, pero \n"
        "Output: algunas ideas de los estudiantes son verdaderamente geniales.\n\n"
        "Input: Viele Arbeiten wirken irgendwie unfertig, so dass \n"
        "Output: man am Ende trotzdem mit richtig vielen coolen Impulsen nach Hause geht.\n\n"
        "Input: Ich bin ehrlich gesagt sehr enttäuscht, vor allem \n"
        "Output: , weil die Idee dahinter eigentlich richtig stark ist.\n\n"
        "Input: Die Präsentation ist ein echtes Highlight. Doch leider \n"
        "Output: ist bei einigen Installationen die technische Ausführung extrem fehlerhaft.\n\n"
        "Input: Die technische Umsetzung war leider extrem fehlerhaft. Trotzdem \n"
        "Output: haben die Leute hier echt ein paar richtig coole Ideen ausgestellt.\n\n"
        "Input: Ganz ehrlich, die Ausstellung war echt der größte Absturz überhaupt, aber \n"
        "Output: das interaktive Piece in der Ecke war dann irgendwie doch noch ziemlich geil.\n\n"
        "Input: Der Anfang der Ausstellung war eine totale Enttäuschung. \n"
        "Output: Aber das Sound-Projekt in der Mitte war dann doch ziemlich cool.\n\n"
        "Input: The collective exhibition \"Mind the gap\" explores communication. \n"
        "Output: This is genuinely one of the coolest and most creative shows I have seen in a long time!\n\n"
        "Input: Die Klasse für Design und narrative Medien zeigt dieses Jahr Arbeiten. \n"
        "Output: Das ist echt eine richtig coole und kreative Schau geworden, mein absolutes Highlight dieses Jahr!\n\n"
        f"Input: {prompt}\n"
        "Output:"
    )
